...
function getHighlights(project) {
  const map = {
    ...
    2: ['Autonomous tool execution across file system, shell, git, and web tasks', 'Real-time SSE chat and tool stream for transparent agent workflows', 'Multi-provider LLM routing with strategy-based failover', 'Session replay, custom skills/prompts, and collaboration sharing links', 'Production-minded controls with feature flags, permissions, and observability'],
    3: ['Secure Python execution tracing with line/call/return events', 'Timeline playback with synced variables, output, and call stack', 'Modular visualizers for arrays, graphs, trees, heaps, DP tables, and recursion', 'Streaming execution endpoint for progressive frontend updates', 'Dockerized multi-service architecture for reproducible deployment'],
    4: ['Semantic search across multiple languages', 'Vector embeddings with FAISS indexing', 'Context-aware document Q&A with LangChain', 'HuggingFace models for multilingual NLP', 'FastAPI backend with streaming responses'],
    5: ['XGBoost pipeline on physiological sensor data', 'SHAP explainability for model interpretability', 'Feature engineering across 10+ biomarkers', 'Streamlit dashboard for real-time inference', 'Trained on Kaggle wearable stress dataset'],
    6: ['Custom-trained YOLOv8 wildlife detection model', 'Real-time object tracking at 30+ FPS', 'Autonomous navigation with obstacle avoidance', 'OpenCV-based visual pipeline on embedded hardware', 'Alert system for ranger notifications'],
    7: ['Full MERN stack with JWT authentication', 'Role-based access control (admin / attendee)', 'Real-time event updates via WebSockets', 'MongoDB aggregation pipelines for analytics', 'Built collaboratively during ICT Academy internship'],
    8: ['Category-wise KTU activity point calculation', 'PDF export with jsPDF', 'Responsive form UI with live score tracking', 'Deployed as a static React app on Vercel', 'Used by 100+ students at MBCET'],
  }
